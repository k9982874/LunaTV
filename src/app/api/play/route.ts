...
export async function GET(request: Request) {
  try {
    ...
    const key = `${source}-${id}`;
    const m3u8Url = await redis.hget(key, name);
    if (!m3u8Url) {
      return NextResponse.json(
    ...
    // 处理 m3u8 中的相对链接
    m3u8Content = resolveM3u8Links(m3u8Content, m3u8Url);
 ...
 */
function resolveM3u8Links(m3u8Content: string, baseUrl: string): string {
